...
void evaluateMLAAEdge(inout half4 finalCol, inout half weight, float2 uv, half2 edgeDist, half2 edgeDir, half2 blendFactor, float2 edgePixelOffset, half4 debugColor)
{
	...
	{
		half4 c = EdgeDetectBuffer.SampleLevel(LinearClampSampler, startUV - (PixelOffset.xy * edgeDir) + edgePixelOffset, 0);	
		if (dot(c.xy,edgeDir) > Threshold)
		{
			...
			finalCol += ColorBuffer.SampleLevel(LinearClampSampler, uv + b * ((1-edgeDir) * PixelOffset.xy), 0); 
#endif		
			weight += 1.0;
		}					
	}
	...
	if (len > 1 && negLength >= posLength) 
	{
		half4 c = EdgeDetectBuffer.SampleLevel(LinearClampSampler, endUV + edgePixelOffset, 0);	
		if (dot(c.xy,edgeDir) > Threshold)
		{	
#ifdef DEBUG_COLOURS
			finalCol += debugColor;
...
{
	half2 edges = EdgeDetectBuffer.SampleLevel(LinearClampSampler, uv, 0).xy;	
	half2 edgesT = EdgeDetectBuffer.SampleLevel(LinearClampSampler, uv - float2(0.0f, PixelOffset.y), 0).xy;
	half2 edgesL = EdgeDetectBuffer.SampleLevel(LinearClampSampler, uv - float2(PixelOffset.x, 0.0f), 0).xy;	
	
	half threshold = Threshold;
	half4 finalCol = 0;
	half weight = 0;

	half4 dist = EdgeLengthBuffer.SampleLevel(LinearClampSampler, uv, 0);
	half4 distT = EdgeLengthBuffer.SampleLevel(LinearClampSampler, uv - float2(0.0f, PixelOffset.y), 0);
	half4 distL = EdgeLengthBuffer.SampleLevel(LinearClampSampler, uv - float2(PixelOffset.x, 0.0f), 0);	
	
	half4 baseColour = ColorBuffer.SampleLevel(LinearClampSampler, uv, 0);
